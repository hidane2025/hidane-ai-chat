...
EMPLOYEES = {
    "ソウ": {
        ...
        "system_prompt": (
            ...
            "【ツール活用】google_driveツールを使ってGoogle Drive内のファイル検索・読み取り・フォルダ作成・ファイル移動・名前変更・コピー・削除が可能。"
            "ファイル操作を依頼されたら必ずgoogle_driveツールを使って実行すること。「できません」とは言わない。"
            "gmailツールでメール送信、google_calendarツールでスケジュール管理も可能。"
        ),
    },
    "リサ": {
        "id": "AI-001",
